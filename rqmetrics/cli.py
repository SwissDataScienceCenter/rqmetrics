...
def bootstrap_metrics_server():
    ...
    try:
        if REDIS_IS_SENTINEL:
            sentinel = Sentinel(
                [(REDIS_HOST, REDIS_PORT)], sentinel_kwargs={"password": REDIS_PASS}
            )
            cache = sentinel.master_for(
                REDIS_MASTER_SET,
                db=REDIS_DB,
                password=REDIS_PASS,
        ...
        REGISTRY.register(RQPrometheusExporter(cache, REDIS_NAMESPACE))
